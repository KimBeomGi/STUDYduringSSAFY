...
# [문제풀이]
# 0. P = A사의 리터 당 가격(원), Q = B사의 기본요금(원), R = B사의 기본제공 수도량(L), S=B사의 리터 당 가격(원), W = 종민이의 수도사용량(L)
# 1-1. A사 이용 시 지불 가격 = 사용량 W(L) X P(원)
# 1-2. B사 이용 시 지불 가격 = R리터 초과 시 Q + (W-R)*S
# 1-3. B사 이용 시 지불 가격 = R리터 이하 시 Q

T = int(input())

for x in range(1, T+1):                              # 테스트케이스를 1~T횟수 만큼 반복문 돌리기
