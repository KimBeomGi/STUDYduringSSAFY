...
while i:
    ...
    headers = {"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Whale/3.21.192.15 Safari/537.36", "Accept-Language": "ko-KR,ko;q=0.8,en-US;q=0.5,en;q=0.3"}  #Accept-Language를 통해 해결
    res = requests.get(url, headers=headers)
    res.raise_for_status()
    soup = BeautifulSoup(res.text, "lxml")
    # print(res.text)
    last_page = soup.find("a", attrs={"class":re.compile("btn-last disabled")})
    if last_page == None or i == int(last_page.get_text())+1:
        break
