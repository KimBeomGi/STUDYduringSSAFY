...
# return 만으로도 가능할 수 있겠지만, print()를 이용하는 것이 먼저 생각났기에 이 방식으로 이용해봤음.
# 재귀함수 생성
def repeat(N, i):       # 인자로 N, i를 받아서 재귀횟수와 ____을 몇 번 출력할 것인지 알려주기
    # 마지막 대답은 다르므로 N==0일 때의 대답을 생성
    if N == 0:
        print("____"*i+'"재귀함수가 뭔가요?"')
        print("____"*i+'"재귀함수는 자기 자신을 호출하는 함수라네"')
        print("____"*i+"라고 답변하였지.")
        return
    # 마지막 대답이 아니면 똑같은 말이 계속되니까 재귀로 생성
    else:
        print("____"*i+'"재귀함수가 뭔가요?"')
