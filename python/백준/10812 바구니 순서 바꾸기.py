...
#     basket[i-1:k-1], basket[k-1:j-1+1] = basket[k-1:j-1+1], basket[i-1:k-1]
#     # print(basket)
#     # print('------')
# print(basket)
...
# a = [1,2,3,4,5,6]
# a[0:3], a[3:6] = a[3:6], a[0:3]
# print(a)




# basket[1-1:4-1], basket[4-1:6-1+1] = basket[4-1:6-1+1], basket[1-1:4-1]
# basket[0:3], basket[3:6] = basket[3:6], basket[0:3]
...
for _ in range(M):
    ...
    j = j-1
    k = k-1
    #  슬라이싱을 이용해보려 했으나 뭔가 삐꾸남.
    # basket[i-1:k-1], basket[k-1:j-1+1] = basket[k-1:j-1+1], basket[i-1:k-1]
    basket = basket[0:i] + basket[k:j+1] + basket[i:k] + basket[j+1:]
