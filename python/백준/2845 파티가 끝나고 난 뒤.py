...
L, P = map(int, input().split())                # 1m^2당 사람의 수 L, 넓이 P
article_num= list(map(int,input().split()))     # 각 기사에 실린 숫자.
people = L*P
for i in range(5):                              # 5개의 기사가 있으므로.
    article_num[i] = str(article_num[i] - people)
print(' '.join(article_num))