...
N = int(input())
for land in range(N):
    length, *battle_field = map(int, input().split())
    half = length//2
    # nation = [0]*(2**31+1)
    # for army in battle_field:
    #     nation[army] +=1
    army_dict = {}
    army_num = []
    ...
    max_count = 0
    max_army = -1
    for army in army_num:
        if max_count < army_dict[army]:
            max_count = army_dict[army]
            max_army = army
        if army_dict[army] > half:
            max_army = army
            pass
        elif max_count == army_dict[army]:
            max_army = 'SYJKGW'
...
for _ in range(n):
    ...
    if threshold==0:
        continue
    mil_num,count=Counter(line[1:]).most_common(1)[0]
    if count>threshold:
        print(mil_num)
    else:
        print('SYJKGW')