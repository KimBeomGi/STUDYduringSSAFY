...
# [문제풀이]
# 0. 겹치는 카드가 있으면 'ERROR'를 출력, 아니라면 각 무늬마다 필요한 카드의 수를 출력

def find_howmany(problem):
    for i in range(0, len(problem), 3):
        if problem[i:i+3] in problem[i+3:]:
            return f'#{testcase} {"ERROR"}'
    S = [0]*13
    D = [0]*13
    H = [0]*13
    C = [0]*13
    for i in range(0, len(problem), 3):
        if problem[i] == 'S':
            if problem[i+1] == '0':
                S[int(problem[i+2])-1] += 1
            elif problem[i+1] == '1':
                S[int(problem[i+1]+problem[i+2])-1] += 1
        elif problem[i] == 'D':
            if problem[i+1] == '0':
                D[int(problem[i+2])-1] += 1
            elif problem[i+1] == '1':
                D[int(problem[i+1]+problem[i+2])-1] += 1
        elif problem[i] == 'H':
            if problem[i+1] == '0':
                H[int(problem[i+2])-1] += 1
            elif problem[i+1] == '1':
                H[int(problem[i+1]+problem[i+2])-1] += 1
        elif problem[i] == 'C':
            if problem[i+1] == '0':
                C[int(problem[i+2])-1] += 1
            elif problem[i+1] == '1':
                C[int(problem[i+1]+problem[i+2])-1] += 1
    s = S.count(0)
    d = D.count(0)
    h = H.count(0)
    c = C.count(0)
    return f'#{testcase} {s} {d} {h} {c}'

T =int(input())
for testcase in range(1, T+1):
    problem = input()
    answer = find_howmany(problem)
    print(answer)
