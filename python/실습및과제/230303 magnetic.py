...
import sys
sys.stdin = open('230303 magnetic.txt','r')

# [문제풀이]
# 0. N극은 1, S극은 2
# 1. 교착 상태는 N극과 S극이 같은 열에서 만나게 되버릴 때 발생함.
# 2-1. 따라서 1을 발견했을 때 2가 해당 열의 아래에 있으면 그 때 는 교착 발생
# 2-2. 2-1의 경우 확인시 COUNT + 1해주기

T = 10
for testcase in range(1, T+1):
    N = int(input())                                    # 크기 N입력받기
    magent_mt = [list(map(int, input().split())) for _ in range(100)]
    deadlock = 0                                        # 교착상태 갯수
    for row in range(N-1):                              # 마지막 행은 확인 할 필요 없음
        for column in range(N):                         # 열 위치 
            if magent_mt[row][column] == 1:             # 찾는 위치에 N극인 1이 발견이 되면
                for nr in range(row+1, N):              # N극의 열에서 밑에 S극이 발견가능한지 확인
                    if magent_mt[nr][column] == 1:      # N극 찾으라니까 N극이 있으면 이번에는 종료. 어차피 다음 N극에서 찾으면 됨.
                        break                           # 종료.(다음 N극에서 찾을 거임)
                    elif magent_mt[nr][column] == 2:    # N극은 못찾았고, S극을 찾게 되었다면
                        deadlock += 1                   # 교착상태 추가하고
                        break                           # 종료하기
