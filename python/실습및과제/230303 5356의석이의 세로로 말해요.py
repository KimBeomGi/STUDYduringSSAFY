import sys
sys.stdin = open('230303 5356의석이의 세로로 말해요.txt','r')
...
# [문제풀이]
# 1. 세로로 읽게 읽는 방식을 list[a][b]에서 list[b][a]로 만들어주도록 하면서 빈칸이 있는 경우는 넘겨야하므로
# 1-1. try except를 이용해보자.

T = int(input())
for testcase in range(1, T+1):
    s_matrix = [list(input()) for _ in range(5)]            # 문자열의 행렬을 입력받아주기
    print(f'#{testcase}', end = ' ')                        # testcase를 출력해주고 답을 출력해보도록 하자.
    for column in range(15):                                # 길이가 1이상 15이하인 문자열이므로 최대 15열까지 읽자.
        for row in range(5):                                # 문자열이 총 5개 있으니까~
            try: print(f'{s_matrix[row][column]}', end='')  # try를 이용해 s_matrix를 세로로 읽는데
            except: continue                                # 어 안되네? 하면 다음으로 넘어가기
    print()