import sys
sys.stdin = open('2230403 5248 그룹 나누기.txt')
...
for testcase in range(1,T+1):
    ...
    p = [x for x in range(N+1)]             # 0~N 까지 들어있는 리스트를 생성
    for i in range(0, M*2, 2):
        union(p[tmp[i]], p[tmp[i+1]])       # 리스트 i값과 i+1값을 합치기
    
    for j in range(1, N+1):
        leader.add(find_set(j))

    print(f'#{testcase} {len(leader)}')