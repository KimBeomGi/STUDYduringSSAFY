...
def article_list(request):
    if request.method == 'GET':
        ...
        if serializer.is_valid(raise_exception=True):
            serializer.save()
        # 저장 되었는지 안되었는지만 알려줘도 되겠지만...
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        
        # 아래 return 은 위 serializer.is_valid(raise_exception=True): raise_exception=True에 의해 불필요
...
def comment_list(request):
    # 전체 댓글 목록 조회해서 반환 >> 댓글들 직렬화 해서 반환
...
def article_comment_list(request, article_pk):
    ...
    if request.method == 'POST':
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            # Serializer에 read_only_fields에 article을 설정해줬기 대문에
            # validation 검사에서 article은 빠짐
